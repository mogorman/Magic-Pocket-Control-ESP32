...
// polling loop.
//
// Adaptation from upstream: the I2C port is a parameter (upstream hard-codes
// port 0). On the M5Stack Core2 the IMU sits on I2C_NUM_1 (SDA=G21, SCL=G22),
// so the port-1 FIFO address and the I2C_EXT1 clock/reset bits are used.
...
// The I2C port in use. mini_i2c_write_txfifo() (in the IRAM workaround header)
// needs this to pick the right FIFO address, and it runs in ISR context where it
// cannot otherwise know the port.
int mini_i2c_port = 0;
...
esp_err_t IRAM_ATTR i2c_conf_pins(i2c_port_t i2c_num, int sda_io_num, int scl_io_num,
    ...
    if (scl_io_num >= 0) {
        ...
        gpio_set_drive_capability(scl_io_num, GPIO_DRIVE_CAP_0);
...
esp_err_t mini_i2c_init(int i2c_num, int sda_pin, int scl_pin, int freq) {
    ...
    i2c_ctx.hal.dev = I2C_LL_GET_HW(i2c_num);
    i2c_ctx.status = I2C_STATUS_IDLE;
    if ((err = i2c_conf_pins((i2c_port_t)i2c_num, sda_pin, scl_pin, 1, 1) != ESP_OK)) {
        return err;
    }
    ...
    i2c_hal_disable_intr_mask(&i2c_ctx.hal, I2C_LL_INTR_MASK);
    i2c_hal_clr_intsts_mask(&i2c_ctx.hal, I2C_LL_INTR_MASK);
    esp_intr_alloc(i2c_periph_signal[i2c_num].irq, ESP_INTR_FLAG_IRAM, i2c_isr_handler, NULL,
                   &i2c_ctx.int_hndl);
    return ESP_OK;
}
...
esp_err_t IRAM_ATTR mini_i2c_master_clear_bus(i2c_port_t i2c_num) {
    ...
    // synchronize the bit stream with, Either, the ACK bit, Or 1 bit to correctly generate a STOP
    // condition.
...
esp_err_t IRAM_ATTR mini_i2c_hw_fsm_reset() {
    i2c_port_t i2c_num = (i2c_port_t)mini_i2c_port;
    ...
    mini_i2c_disable_hw(mini_i2c_port);
    mini_i2c_master_clear_bus(i2c_num);
    mini_i2c_enable_hw(mini_i2c_port);
