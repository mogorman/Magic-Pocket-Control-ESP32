...
// Images
#include "Images/MPCSplash-Waveshare.h"
...
// Last touch tap coordinates (screen space). -1 means "no tap this loop". The
// per-screen code hit-tests these to drive on-screen buttons.
int tapped_x = -1;
int tapped_y = -1;
...
void Screen_GyroLog(bool forceRefresh = false)
{
  ...
  else
  {
    ...
    // We read the QMI8658 directly (gyro in deg/s, accel in g) so the numbers
    // shown match what the user sees for orientation. The values are converted to
    // the same units the GCSV logger records (gyro rad/s, accel g) below.
...
void setup() {
  ...
  keyA.attachClick([]() { keyAPressed = true; });
...
void loop() {
  ...
  */
  ...
  // ---- Input: touch (tap) + three user buttons (A=GPIO0, B=GPIO16, C=GPIO18) ----
  btnAPressed = false;
  btnBPressed = false;
  ...
  // --- Button A (GPIO0): up / select ---
  // Drives the per-screen A/B option handlers (ISO, shutter, white balance,
  // codec, resolution, etc.) via the shared btnAPressed flag.
  keyA.tick();
  if(keyAPressed)
  {
    keyAPressed = false;
    DEBUG_DEBUG("Button A (GPIO0) > UP / SELECT");
    btnAPressed = true;
  }

  // --- Button B (GPIO16): record start/stop on the main screens, else down ---
  keyB.tick();
  if(keyBPressed)
  {
    keyBPressed = false;
    ...
    {
      DEBUG_DEBUG("Button B (GPIO16) > RECORD START/STOP");
      if(BMDControlSystem::getInstance()->hasCamera())
      {
        auto camera = BMDControlSystem::getInstance()->getCamera();
  ...
  // --- Button C (GPIO18): next / previous screen ---
  keyC.tick();
  if(keyCPressed)
  {
    keyCPressed = false;
    ...
    switch(connectedScreenIndex)
    {
      ...
      case Screens::ShutterAngleSpeed: connectedScreenIndex = Screens::WhiteBalanceTintWB; break;
      case Screens::WhiteBalanceTintWB: connectedScreenIndex = Screens::WhiteBalanceTintT; break;
      case Screens::WhiteBalanceTintT:  connectedScreenIndex = Screens::Codec; break;
      case Screens::Codec:      connectedScreenIndex = Screens::Resolution; break;
      case Screens::Resolution: connectedScreenIndex = Screens::Framerate; break;
      case Screens::Framerate: connectedScreenIndex = Screens::Media; break;
      case Screens::Media:     connectedScreenIndex = Screens::Lens; break;
      case Screens::Lens:      connectedScreenIndex = Screens::GyroLog; break;
      default: break;
    }
    lastRefreshedScreen = 0;
  }
